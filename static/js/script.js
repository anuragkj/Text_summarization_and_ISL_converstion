...
let sub =  document.getElementById('submit');
  sub.addEventListener('click', async ()=>
  {
    ...
    // ########################################################################################
    const sentences = input.split(/[.!?]/g).filter(sentence => /^[A-Za-z0-9 ]+$/.test(sentence));
    const arrayLength = sentences.length;
    for (var i = 0; i < arrayLength; i++) {
        console.log("in console", sentences[i]);
        // ajax request to get the response from flask in json and play the words
        await $.ajax({
          url:'/',
          type:'POST',
          data:{text:sentences[i]},
...
function convert_json_to_arr(words)
{
    ...
    //wordArray = wordArray.filter(element => /^[A-Za-z0-9]+$/.test(element));
